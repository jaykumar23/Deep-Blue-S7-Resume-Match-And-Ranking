...
def user_login(request):
    if request.method == 'PUT' or request.method == 'POST':
        ...
        if check_password(password, userobj.password):
            ...
            # Bad Login
            return JsonResponse({"status": False, "Desc": "User id or Password did not match"})
...
def user_profile(request):
    if request.method == 'PUT' or request.method == 'POST':
        data = request.data.get('')
        try:
            Signup.objects.filter(id=data.id).update(email=data.email, first_name=data.first_name,
                                                     last_name=data.last_name, gender=data.gender,
                                                     mobile_no=data.mobile_no, updated_at=timezone.now())
...
def upload_resume(request, userid):
    if request.method == 'PUT' or request.method == 'POST':
        ...
        if not existing_resume:
            Resume.objects.create(applicant=userid, resume=data, created_at=timezone.now(), updated_at=timezone.now())
...
def upload_job(request, id):
    if request.method == 'PUT' or request.method == 'POST':
        data = request.data.get('')
        existing_job = Job.objects.filter(applicant=data.userid)
        if not existing_job:
            Job.objects.create(recruiter=data.userid, designation=data.designation, location=data.location,
                               start_date=data.start_date, duration=data.duration, stipend=data.stipend,
                               apply_by=data.apply_by, job_description=data.job_description,
                               skills_required=data.skills_required, who_can_apply=data.who_can_apply, perks=data.perks,
                               number_of_openings=data.number_of_openings, created_at=timezone.now(),
                               updated_at=timezone.now())
            return HttpResponse('Job Uploaded Successfully')
        else:
            Job.objects.filter(id=id).update(designation=data.designation, location=data.location,
                                             start_date=data.start_date, duration=data.duration, stipend=data.stipend,
                                             apply_by=data.apply_by, job_description=data.job_description,
                                             skills_required=data.skills_required, who_can_apply=data.who_can_apply,
                                             perks=data.perks, number_of_openings=data.number_of_openings,
                                             updated_at=timezone.now())
